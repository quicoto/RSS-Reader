...
while ($feed = $feeds->fetch_assoc()) {
  ...
  $query_old_items = "DELETE FROM ".$table_items." WHERE ID in (SELECT ID from (select ID from ".$table_items." WHERE DATEDIFF(NOW(),date) > ".$delete_old_items_days . " AND is_read = 1 AND feed_id = ".$feed['id']." ORDER BY ID DESC LIMIT 10, 100) x)";
